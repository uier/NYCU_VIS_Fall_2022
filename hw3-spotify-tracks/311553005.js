...
class SpotifyTracks {
  ...
  render() {
    ...
    const colorAxis = "track_genre";
    const cScale = d3
      .scaleOrdinal()
      .domain(this.data.map((d) => d[colorAxis]))
      .range(palette);
    const xDragging = {};
    ...
    legendContainer
      .selectAll("circle")
      .data([...new Set(this.data.map((d) => d[colorAxis]))])
      .enter()
      .append("circle")
      .attr("cx", 6)
      .attr("cy", (_, i) => 15 + i * 25)
...
async function main() {
  ...
  const axes = raw.columns.filter((d) => features.includes(d));
  ...
  const chart = new SpotifyTracks(dataset, layout, axes);
  // console.log(dataset);
}
